  ...
  return (
    <>
      <article className="mx-auto max-w-4xl px-6 py-14 sm:py-20">
        ...
        {images[0] ? <img src={images[0]} alt="" className="mt-10 aspect-[16/9] w-full rounded-2xl border border-[var(--tk-line)] object-cover" /> : null}
...
function ListingDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-5 sm:px-8 lg:px-[60px] py-14 sm:py-20">
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <>
      <section className="mx-auto grid max-w-[var(--editable-container)] gap-10 px-5 sm:px-8 lg:px-[60px] py-14 sm:py-20 lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <BackLink task="classified" />
          <div className="mt-7 rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] p-7 shadow-[0_8px_32px_rgba(21,15,65,0.08)]">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              {phone ? <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-xl bg-[var(--tk-accent)] px-5 py-2.5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90"><Phone className="h-4 w-4" /> Call now</a> : null}
              {email ? <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-xl border border-[var(--tk-line)] px-5 py-2.5 text-sm font-semibold transition hover:border-[var(--tk-accent)]"><Mail className="h-4 w-4" /> Email</a> : null}
            </div>
          </div>
        </aside>
        <article className="min-w-0">
...
// ----- Bookmark: a premium curated resource (completely rebuilt) -----
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <>
      {/* Premium hero band */}
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #150f41, #2a1f6e 50%, #4760e6)' }}>
        <div className="mx-auto max-w-[var(--editable-container)] px-5 sm:px-8 lg:px-[60px] py-16 sm:py-24">
          <h1 className="editable-display max-w-3xl text-balance text-4xl font-semibold leading-[1.06] tracking-[-0.03em] text-white sm:text-5xl lg:text-[3.4rem]">{post.title}</h1>
          {domain ? (
            <div className="mt-6 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-white/90" style={{ background: 'rgba(255,255,255,0.2)' }}>
          ...
          {website ? (
            <div className="mt-8">
              <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-semibold text-[#4760e6] transition hover:opacity-90">
      ...
      <div className="border-b border-[var(--tk-line)] bg-[var(--tk-surface)]">
        <div className="mx-auto flex max-w-[var(--editable-container)] flex-wrap items-center gap-x-6 gap-y-2 px-5 sm:px-8 lg:px-[60px] py-4 text-sm">
          <span className="inline-flex items-center gap-2 font-medium text-[var(--tk-text)]">
            <Bookmark className="h-4 w-4 text-[var(--tk-accent)]" />
            {taskConfig?.label || 'Collection'}
          </span>
          {domain ? (
            <>
              <span className="h-1 w-1 rounded-full bg-[var(--tk-muted)] opacity-50" />
              <span className="text-[var(--tk-muted)]">{domain}</span>
            </>
          ) : null}
          <span className="h-1 w-1 rounded-full bg-[var(--tk-muted)] opacity-50" />
      ...
      <div className="mx-auto max-w-[var(--editable-container)] px-5 sm:px-8 lg:px-[60px] py-14 sm:py-20">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px]">
          {/* Main body */}
          <article className="min-w-0">
            <h2 className="editable-display text-2xl font-semibold tracking-[-0.02em]">About this resource</h2>
            {leadText(post) ? <p className="mt-4 text-lg leading-8 text-[var(--tk-muted)]">{leadText(post)}</p> : null}
            <BodyContent post={post} />
            {tags.length ? (
              <div className="mt-8 flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="inline-flex items-center gap-1.5 rounded-full bg-[var(--tk-accent-soft)] px-3.5 py-1.5 text-xs font-medium text-[var(--tk-accent)]">
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6">
              ...
              <div className="mt-4 grid gap-2.5 text-sm text-[var(--tk-muted)]">
                <p className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-green-500" /> Verified resource</p>
                {domain ? <p className="inline-flex items-center gap-2"><Globe2 className="h-4 w-4 text-[var(--tk-accent)]" /> {domain}</p> : null}
                <p className="inline-flex items-center gap-2"><Tag className="h-4 w-4 text-[var(--tk-accent)]" /> {taskConfig?.label || 'Collection'}</p>
              </div>
            </div>
      ...
      {related.length ? (
        <section className="border-t border-[var(--tk-line)]">
          <div className="mx-auto max-w-[var(--editable-container)] px-5 sm:px-8 lg:px-[60px] py-14 sm:py-16">
            <div className="flex items-center justify-between">
              <h2 className="editable-display text-2xl font-semibold tracking-[-0.02em]">More from this collection</h2>
          ...
          {fileUrl ? (
            <div className="mt-10 overflow-hidden rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)]">
              <div className="flex items-center justify-between gap-3 border-b border-[var(--tk-line)] p-4">
                ...
                <Link href={fileUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-[var(--tk-accent)] px-4 py-2 text-xs font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90">Download <Download className="h-4 w-4" /></Link>
              </div>
        ...
        <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          {fileUrl ? (
            <div className="rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6 shadow-[0_8px_32px_rgba(21,15,65,0.08)]">
              ...
              <Link href={fileUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--tk-accent)] px-5 py-3 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90">Download <Download className="h-4 w-4" /></Link>
            </div>
          ) : null}
...
// ----- Profile: identity-first (completely rebuilt) -----
function ProfileDetail({ post, related: _related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <>
      {/* Identity hero: cover band with gradient */}
      <div className="relative" style={{ background: 'linear-gradient(135deg, #150f41, #2a1f6e 50%, #4760e6)' }}>
        <div className="h-56 sm:h-72" />
      </div>

      {/* Profile content below the hero */}
      <div className="mx-auto max-w-[var(--editable-container)] px-5 sm:px-8 lg:px-[60px]">
        <div className="grid gap-10 pt-10 lg:grid-cols-[minmax(0,1fr)_380px]">
          {/* Main column */}
          <div className="min-w-0">
            {/* Avatar */}
            <div className="mb-6 flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-[var(--tk-raised)] shadow-[0_8px_32px_rgba(21,15,65,0.08)]">
              {images[0] ? <img src={images[0]} alt="" className="h-full w-full object-cover" /> : <UserRound className="h-10 w-10 text-[var(--tk-muted)]" />}
            </div>

            {/* Name + role */}
            <h1 className="editable-display text-4xl font-semibold leading-[1.05] tracking-[-0.03em] sm:text-5xl">{post.title}</h1>
            {role ? (
              <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[var(--tk-accent-soft)] px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--tk-accent)]">
                {role}
              </div>
            ) : null}
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {website ? (
                <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-[var(--tk-accent)] px-5 py-2.5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90">
                  <Globe2 className="h-4 w-4" /> Website
                </Link>
              ) : null}
              {email ? (
                <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-xl border border-[var(--tk-line)] px-5 py-2.5 text-sm font-semibold transition hover:border-[var(--tk-accent)]">
                  <Mail className="h-4 w-4" /> Email
                </a>
              ) : null}
              {phone ? (
                <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-xl border border-[var(--tk-line)] px-5 py-2.5 text-sm font-semibold transition hover:border-[var(--tk-accent)]">
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6 shadow-[0_8px_32px_rgba(21,15,65,0.08)]">
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--tk-muted)]">Identity info</p>
              <div className="mt-4 grid gap-3 text-sm">
                <div className="flex items-center gap-2 text-[var(--tk-text)]">
                  <UserRound className="h-4 w-4 text-[var(--tk-accent)]" />
                  <span className="font-medium">{post.title}</span>
                </div>
                {role ? (
                  <div className="flex items-center gap-2 text-[var(--tk-muted)]">
                    <Building2 className="h-4 w-4 text-[var(--tk-accent)]" />
                    <span>{role}</span>
                  </div>
                ) : null}
                {website ? (
                  <div className="flex items-center gap-2 text-[var(--tk-muted)]">
                    ...
                    <Link href={website} target="_blank" rel="noreferrer" className="truncate hover:text-[var(--tk-accent)]">{website}</Link>
                  </div>
                ) : null}
                {email ? (
                  <div className="flex items-center gap-2 text-[var(--tk-muted)]">
                    <Mail className="h-4 w-4 text-[var(--tk-accent)]" />
                    <a href={`mailto:${email}`} className="truncate hover:text-[var(--tk-accent)]">{email}</a>
                  </div>
                ) : null}
                {phone ? (
                  <div className="flex items-center gap-2 text-[var(--tk-muted)]">
                    <Phone className="h-4 w-4 text-[var(--tk-accent)]" />
                    <a href={`tel:${phone}`} className="hover:text-[var(--tk-accent)]">{phone}</a>
                  </div>
                ) : null}
                <div className="flex items-center gap-2 text-[var(--tk-muted)]">
                  <CheckCircle2 className="h-4 w-4 text-green-500" />
  ...
  return (
    <section className="mt-10">
      ...
      <div className={`mt-4 grid gap-3 ${large ? 'sm:grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {images.slice(0, large ? 4 : 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-[4/3] rounded-2xl border border-[var(--tk-line)] object-cover" />)}
      </div>
    </section>
  )
}
...
function ContactAction({ website, phone, email, bare = false }: { website?: string; phone?: string; email?: string; bare?: boolean }) {
  ...
  const buttons = (
    <div className={`flex flex-wrap gap-2.5 ${bare ? 'justify-center' : ''}`}>
      {website ? <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-[var(--tk-accent)] px-4 py-2.5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:opacity-90">Website <ExternalLink className="h-4 w-4" /></Link> : null}
      {phone ? <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-xl border border-[var(--tk-line)] px-4 py-2.5 text-sm font-semibold transition hover:border-[var(--tk-accent)]"><Phone className="h-4 w-4" /> Call</a> : null}
...
function RelatedCard({ task, post, grid = false }: { task: TaskKey; post: SitePost; grid?: boolean }) {
  ...
  if (grid) {
    return (
      <Link href={href} className="group block overflow-hidden rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-1 shadow-[0_8px_32px_rgba(21,15,65,0.08)]">
